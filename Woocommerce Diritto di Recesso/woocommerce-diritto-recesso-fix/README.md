...
1. Apri (via FTP/SFTP o file manager dell'hosting) il file `wp-config.php`, che si trova nella cartella principale dell'installazione WordPress.
2. Individua la riga:
   ```php
   /* That's all, stop editing! Happy publishing. */
   ```
3. Subito **prima** di quella riga, aggiungi:
   ```php
   define( 'WCDR_ADMIN_EMAIL', 'altra-email@esempio.it' );
   ```
